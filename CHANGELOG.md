...
- `app/manifest.ts` (the web app manifest) no longer ships the pre-rebrand "Cron jobs over MCP & REST" tagline and an unrelated `#fafafa` theme color — it now reuses the same "Put your agents on a loop" copy as the rest of the site's metadata and matches the actual `#f4f1e8` background.
