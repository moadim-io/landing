...
- A `purpose: "maskable"` entry alongside the existing `app/manifest.ts` icon, so Android's adaptive-icon system crops `app/icon.svg`'s "M" glyph (which already sits inside the maskable safe zone) as full-bleed artwork on "Add to Home Screen" instead of padding it onto a plain system-color circle.
